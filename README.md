...
**Version:** 4.2.0 | **Runtime:** `dist/ActivityLoggerNative.app` | **Operations:** [`docs/MACOS_TCC.md`](docs/MACOS_TCC.md)
...
Version 4.2.0 also writes a temporary private comparison set under `logs/analysis_shadow/`. The existing daily log remains authoritative during this trial. The analysis Markdown uses stable event names, one context heading per change, reversible adjacent-repeat counts, focus and idle transitions, session markers, and hourly continuity markers. It is generated by local Python code only. It does not call an LLM or a network service.
