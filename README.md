...
**Version:** 4.3.0 | **Runtime:** `dist/ActivityLoggerNative.app` | **Operations:** [`docs/MACOS_TCC.md`](docs/MACOS_TCC.md)
...
Version 4.3.0 also writes a temporary private comparison set under `logs/analysis_shadow/`. The existing daily log remains authoritative during this trial. Analysis days through 2026-08-24 use `activitylogger-analysis-v1`. Starting on 2026-08-25 in Europe/Zagreb, new daily shadow files use `activitylogger-analysis-v2`. The fixed local-day boundary prevents one daily file from mixing formats. V2 keeps the exact event records and intent digests while reducing timeline overhead. The analysis Markdown uses stable event names, one context heading per change, reversible adjacent-repeat counts, focus and idle transitions, session markers, and hourly continuity markers. It is generated by local Python code only. It does not call an LLM or a network service.

The planned 4.3.0 deployment restart on 2026-08-24 adds session stop and start markers to that day. Therefore, 2026-08-24 is not eligible as a cutover soak day. Keep the logger running without a restart throughout 2026-08-25. The first possible v2 soak result can be reviewed only after the first 2026-08-26 heartbeat.
...
This integrity check does not assert that the day is complete or that heartbeat coverage is sufficient for cutover. It does not print headings or payloads.
