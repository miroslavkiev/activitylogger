...
mode: collaborative
fps: 30
music: "clean optimistic pulse with soft glass tones and a quiet human close"
captions: open
status: proposal
---
...
Scene 1 (0.0-2.2s): the Review Center assembles across columns 2 to 8. The cursor arrives beside `Create review files` but does not click.
Scene 2 (2.2-6.0s): the question reveals in Sora across columns 7 to 12 as the voice asks it.
...
Scene 1 (0.0-2.8s): a Ciklum Blue field replaces the white opening. `5 consecutive completed calendar days` appears in white.
Scene 2 (2.8-6.0s): a faithful Step 1 view enters. It shows an end-date field with `Illustrative Day 5`, the real `5 days` menu, the fixed `Create review files` button, and `Checking selected days...`.
Scene 3 (6.0-8.0s): the status becomes `Ready to create files for Day 1 through Day 5.` and the button becomes enabled without changing its label.
Scene 4 (8.0-11.0s): the coverage text `File checks do not prove that every activity was captured.` appears and holds while the ready status remains visible. No older or substitute day enters.
...
Scene 1 (0.0-2.5s): the real icon appears at columns 1 to 3. A blank white document card enters across columns 4 to 11.
Scene 2 (2.5-8.5s): example rows reveal in spoken order: app and window, typing and hotkeys, clicks, clipboard change. `Examples shown` remains visible.
Scene 3 (8.5-9.0s): `Local daily Markdown` arrives as a small blue proof label under the page.
...
Scene 1 (0.0-2.8s): `Sensitive plaintext` appears in white on Ciklum Blue with a simple file icon.
Scene 2 (2.8-7.0s): `Configured secure app`, `Secure field`, and `Manual pause` replace one another beside a fixed `Paused` status.
Scene 3 (7.0-9.8s): `No screenshots`, `No audio`, and `No video` enter as three readable labels.
...
Scene 1 (0.0-3.5s): Step 1 enters across columns 1 to 4 with a local file icon and `Prepares files`.
Scene 2 (3.5-7.5s): Step 2 takes the central columns and becomes the largest card. `Review and redact` uses ink text with a teal underline and icon, not teal text on white.
Scene 3 (7.5-11.5s): Step 3 enters across columns 9 to 12, labeled `Trusted tool chosen by you`.
...
Scene 1 (0.0-3.2s): `Illustrative data` appears first and remains visible. `Possible pattern` then arrives in a large white card on a pale canvas.
Scene 2 (3.2-7.2s): `Not exact time` and `Unknown gaps stay unknown` appear in a blue limits card beside it.
Scene 3 (7.2-9.0s): three source chips labeled Day 1, Day 3, and Day 5 reveal under the pattern with short context labels.
Scene 4 (9.0-12.0s): `Day and context linked` holds with `Illustrative data`. No signal line has appeared yet.
...
Scene 1 (0.0-3.0s): a result card labeled `Trusted review tool chosen by you` sits at left. Step 3 of the Review Center sits at right with the closed `Choose a result` menu and `Save review result` button.
Scene 2 (3.0-5.2s): a single accessible teal `#009E84` path labeled `Human review` draws toward the cursor over 2.2 seconds. It stops before reaching the Review Center control. This is the motion peak.
Scene 3 (5.2-8.2s): the cursor opens the real menu, holds all three choices, and selects `Found an idea to try`. The menu also shows `Tried a change` and `No action`.
Scene 4 (8.2-12.0s): the cursor presses `Save review result` and `Review result saved locally.` appears. `You decide` remains the main subject. No task, upload, automation, or automatic return path starts.
...
Scene 1 (0.0-1.5s): the ActivityLogger icon appears at left, above the caption band. The final line reveals across columns 3 to 10.
Scene 2 (1.5-4.0s): the voice completes the line. A single teal period appears and remains still.
