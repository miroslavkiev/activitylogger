...
LOG_DIR            = Path.home() / "scripts" / "activitylogger" / "logs"
    ...
    while True:
        ...
        with _lock:
            ...
            if new_heading != _current_heading:
                _flush_keys()
                if _current_events and _current_heading:
                    _sections.append({
...
def flush_to_file(final: bool = False):
    ...
    with open(filepath, "a", encoding="utf-8") as f:
        if is_new:
            f.write(f"# Work Log — {datetime.now().strftime('%Y-%m-%d')}\n\n")
            f.write("> Auto-generated by Interleaved Logger v4 (AX + Clipboard + Security + Hotkeys)\n\n---\n\n")
...
def main():
    filepath = _get_filepath()
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Interleaved Logger v4 started")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Log file: {filepath}")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] AX_AVAILABLE (PyObjC): {AX_AVAILABLE}")
